...
The Sales CRM (same login `User`, company-wide data, CRM-only RBAC) is specified in `docs/DEVELOPMENT_PLAN.md` Track D. `apps/crm` is the CRM UI. CRM HTTP lives under `/api/crm` (session, contacts, enquiries, follow-ups, clients, payments, tasks, calendar, users, roles, dashboard). Public banquet or travel intake is `POST /api/crm/public/enquiries` (no JWT, `kind` = `banquet` or `travel`): it creates a lead contact when the mobile is new, or a new enquiry on the existing active contact, and stores event or trip details plus notes on the enquiry. Banquet `eventDate` and travel `departureDate` must be today or in the future. The clients module is labeled **Booked** in the CRM UI (API paths and `crm.clients.*` permissions stay the same). The Booked list (`GET /api/crm/clients`) includes each record’s current booking `startsAt` and `endsAt`, and can be filtered by optional `from`/`to` query dates that match overlapping calendar events. Contact and booked views (`GET /api/crm/contacts/:id`) include that party’s enquiries with follow-ups, related payments, and current bookings (calendar events linked to the enquiry). Convert-to-booked requires an event start datetime that is today or later and either an end datetime or a slot (morning / evening / full day), and seeds `CrmCalendarEvent.notes` from the enquiry's latest follow-up note (falling back to the enquiry notes). Booking notes are then editable in the current-booking view (`crm.calendar.update`), so the final menu can be appended, and can be copied to the clipboard. Linked booking events appear as **Booked** items on the main calendar and show the slot on the chip when one is set. Calendar sits in the **Overview** nav group under Dashboard; clicking a day on it asks whether to add a **Reminder** (title required, optional description, remind-at time, optional linked contact), an enquiry, a follow-up, a booking, a payment, or a task (options filtered by create permission). Module forms open in their own tab with the picked day prefilled. Reminders also live under **Work** next to Tasks: table, card, and calendar views, with add/edit/remove for title, description, remind-at time, and an optional contact. An enquiry and its booking are removed together in both directions: removing the enquiry soft-deletes its booking event, and removing the booking soft-deletes its enquiry, so neither side is left orphaned. Once a linked payment is `paid`, neither side can be deleted. Add, edit, and record-view screens open in a right-side sheet; remove and stage-move confirmations stay centered dialogs.
...
**CRM (Track D):** `Role`, `Permission`, `RolePermission`, `UserRole`, `CrmContact`, `CrmEnquiry`, `CrmFollowUp`, `CrmClient`, `CrmPayment`, `CrmTask`, `CrmCalendarEvent`. Do not add `tenantId`. `CrmEnquiry.status` is the eight-stage sales pipeline: New → Contacted → Qualified → Discussion → Quotation Sent → Negotiation → Schedule Meeting / Site Visit → Closed. Do not add extra stages. `CrmEnquiry.dueDate` is an exact calendar date that must be today or in the future (the form calendar has shortcuts for 7 days / 1 month / 3 months / 6 months from today and past days cannot be picked). Convert-to-booked `startsAt` must also be today or later. `dueDateWindow` remains on existing rows only. `CrmEnquiry.nextFollowupDate` is the current next-contact date; `CrmFollowUp` is the history of notes/status plus `nextFollowupDate`. Overdue means `nextFollowupDate` has passed and the enquiry is not closed. Follow-ups for today are open enquiries whose `nextFollowupDate` falls on the current calendar day; the dashboard card opens Follow-ups with When = Today. A scheduled follow-up (an open enquiry's `nextFollowupDate`) also appears on the main Calendar module as a `kind=followup` item that opens the related contact; the Follow-ups module keeps its own calendar of new-enquiry due dates and follow-ups, plus a timeline of lead follow-up and closure. The Calendar module has day, week, and month views (month is the default); the loaded `from`/`to` range follows the selected view, and every view fits the viewport width on mobile instead of scrolling sideways. `CrmPayment` uses `PaymentType` (`INCOME` / `EXPENSE`) and `PaymentMode` (`CASH`, `UPI`, `CARD`, `BANK_TRANSFER`, `CHEQUE`) only. Public HTML walkthroughs on `apps/crm` (same iframe pattern as finance `/guide`): `/banquet` (hall enquiry → booking), `/real-estate` (2BHK enquiry → token), `/freedom` (myfinancefreedom.com sales agent → Freedom Planner onboarding). These routes are unauthenticated.
...
| Profile | Retirement age, dependents with per-person details (name, relationship, date of birth, gender, occupation), inflation, employment, currency |
