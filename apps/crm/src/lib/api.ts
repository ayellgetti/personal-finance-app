...
async function sendJson<T>(path: string, options: RequestOptions = {}): Promise<T> {
  const { body, token, headers: headerInit, ...rest } = options;
...
export async function api<T>(path: string, options: RequestOptions = {}): Promise<T> {
  const { skipAuth: skipAuthOption, ...sendOptions } = options;
  const skipAuth = skipAuthOption ?? isAuthFree(path);
  const token = skipAuth ? null : await resolveAccessToken(sendOptions.token);

  try {
    return await sendJson<T>(path, { ...sendOptions, token });
  } catch (error) {
    const canRetry =
      !skipAuth &&
    ...
    return sendJson<T>(path, { ...sendOptions, token: nextToken });
  }
}
